...
CONTAINS
  ...
  SUBROUTINE initialise_biogem(dum_saln0, dum_rhoair, dum_cd, dum_ds, &
    ...
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ALLOCATE(bio_part(nt_sed_all,n_i,n_j,n_k),STAT=alloc_error)              ; bio_part = 0.0
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ALLOCATE(bio_remin(n_ocn,n_i,n_j,n_k),STAT=alloc_error)             ; bio_remin = 0.0
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ALLOCATE(bio_settle(nt_sed_all,n_i,n_j,n_k),STAT=alloc_error)            ; bio_settle = 0.0
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ALLOCATE(bio_part_red(nt_sed_all,nt_sed_all,n_i,n_j),STAT=alloc_error)        ; bio_part_red = 0.0
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ...
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ALLOCATE(int_bio_part_timeslice(nt_sed_all,n_i,n_j,n_k),STAT=alloc_error)              ; int_bio_part_timeslice = 0.0
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ALLOCATE(int_bio_settle_timeslice(nt_sed_all,n_i,n_j,n_k),STAT=alloc_error)            ; int_bio_settle_timeslice = 0.0
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ...
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ALLOCATE(int_sfcsed1_timeslice(nt_sed_all,n_i,n_j),STAT=alloc_error)                   ; int_sfcsed1_timeslice = 0.0
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ALLOCATE(int_focnsed_timeslice(nt_sed_all,n_i,n_j),STAT=alloc_error)                   ; int_focnsed_timeslice = 0.0
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ...
    ALLOCATE(force_flux_sed(nt_sed_all,n_i,n_j),STAT=alloc_error)    ; force_flux_sed = 0.0
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ALLOCATE(force_flux_sed_I(nt_sed_all,n_i,n_j),STAT=alloc_error)  ; force_flux_sed_I = 0.0
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ALLOCATE(force_flux_sed_II(nt_sed_all,n_i,n_j),STAT=alloc_error) ; force_flux_sed_II = 0.0
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
...
!!! *** TESTING ***
  ...
  SUBROUTINE step_biogem(dum_dts, dum_genie_clock, dum_sfcatm1, dum_sfxatm1, &
    ...
    REAL,DIMENSION(nt_sed_all,n_i,n_j,n_k)::locijk_fpart                ! local particulate tracer flux array
    ...
    REAL,DIMENSION(nt_sed_all,n_i,n_j)::locij_focnsed                   ! local ocn->sed change (sed tracer currency), units (mol)
                   ...
                   If (flag_sedgem) then
                      ...
                      DO ls=1,nt_sed
                         ...
                         do loc_i=1,loc_tot_i
                            ...
                            if (lo > 0) then
                               if (sed_type(is_iss(ls)) == par_sed_type_scavenged) then
                                  if ((is_iss(ls) == iss_POM_S) .OR. (sed_dep(is_iss(ls)) == iss_POM_S)) then
                                     locij_fsedocn(l2io(lo),i,j) = locij_fsedocn(l2io(lo),i,j) + &
                                          & loc_conv_ls_lo(lo,ls)*bio_settle(is_iss(ls),i,j,loc_k1)
                                  else
                                     locij_fsedocn(l2io(lo),i,j) = locij_fsedocn(l2io(lo),i,j) + &
                                          & par_scav_fremin*loc_conv_ls_lo(lo,ls)*bio_settle(is_iss(ls),i,j,loc_k1)
                                  end if
                               else
                                  locij_fsedocn(l2io(lo),i,j) = locij_fsedocn(l2io(lo),i,j) + &
                                       & loc_conv_ls_lo(lo,ls)*bio_settle(is_iss(ls),i,j,loc_k1)
             ...
             IF (n_k >= loc_k1) THEN
                ...
                IF (force_restore_ocn_select(io_ALK) .AND. force_flux_ocn_select(io_ALK)) THEN
                   ...
                   IF (force_restore_atm_select(ia_pCO2) .AND. (par_force_invert_ohmega < const_real_nullsmall)) THEN
                      ...
                      loc_force_target = force_restore_atm(ia_pCO2,i,j)/par_atm_force_scale_val(ias_pCO2)
  ...
  SUBROUTINE diag_biogem_timeslice(dum_dts, dum_genie_clock, dum_sfcatm1, dum_sfxatm1, &
    ...
    REAL,DIMENSION(nt_sed_all,n_i,n_j)::locij_focnsed                   ! local ocn->sed change (sed tracer currency) (mol)
