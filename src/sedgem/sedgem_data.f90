...
!!!real,dimension(n_i,n_j,n_sed_tot)::loc_ijk
    ...
    DO i = 1,n_i
       DO j = 1,n_j
          if (sed_save_mask(i,j)) then
             ...
             call check_iostat(ios,__LINE__,__FILE__)
             write(unit=out,fmt='(A1,2A11,A8,A8,2A8,7A10,A10,A8,A10,A8,A10,5A10,12A10)',iostat=ios) &
                  & '%',                                          &
                  & '   time_kyr',                                &
                  & '  age_kyrBP',                                &
                  & '     D_m',                                   &
  ...
  SUBROUTINE sub_sedgem_save_sedcoreenv( &
    ...
    real::loc_age                                                !
    real::loc_sed_tot_wt                                         !
    REAL,DIMENSION(nt_sed_all)::loc_sed                               !
    ...
    call check_iostat(ios,__LINE__,__FILE__)
    write(unit=out,fmt='(1X,2f11.4,f8.1,f8.3,2f8.3,7f10.3,f10.3,f8.3,f10.3,f8.3,f10.3,5f10.3,12f10.3)',iostat=ios) &
  ...
  SUBROUTINE sub_data_save_seddiag_GLOBAL(dum_dtyr,dum_sfcsumocn)
    ...
    REAL, DIMENSION(nt_sed,n_i,n_j) :: loc_fsed
    REAL, DIMENSION(nt_sed,n_i,n_j) :: loc_fdis
    ...
    do is = 1, nt_sed
       iss = is_iss(is)
       loc_fsed(is,:,:) = (sed_fsed(iss,:,:) + sed_fsed_OLD(iss,:,:))/loc_dt
       loc_fdis(is,:,:) = (sed_fdis(iss,:,:) + sed_fdis_OLD(iss,:,:))/loc_dt
    end do
    ! calculate d13C
    DO i=1,n_i
       DO j=1,n_j
          if (loc_mask_reef(i,j)*loc_fsed(is_CaCO3,i,j) > const_real_nullsmall) then
             loc_tot  = loc_fsed(iss_is(sed_dep(iss_CaCO3_13C)),i,j)
          ...
          if (loc_mask_reef(i,j)*loc_fsed(is_POC,i,j) > const_real_nullsmall) then
             loc_tot  = loc_fsed(iss_is(sed_dep(iss_POC_13C)),i,j)
  ...
  SUBROUTINE sub_sedgem_save_sedcore()
    ...
    ALLOCATE(loc_sed_save(nt_sed_all,n_i,n_j,0:n_sed_tot),STAT=alloc_error)
    call check_iostat(alloc_error,__LINE__,__FILE__)
