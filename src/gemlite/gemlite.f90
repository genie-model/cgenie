MODULE gemlite
  ...
  PRIVATE
  PUBLIC :: initialise_gemlite
  PUBLIC :: step_gemlite
  PUBLIC :: gemlite_ts
  PUBLIC :: gemlite_cycleinit
  PUBLIC :: gemlite_climate
  PUBLIC :: cpl_comp_gemglt
  PUBLIC :: cpl_comp_gltgem_d
  PUBLIC :: cpl_comp_gltgem_dsum
CONTAINS

  ! ******************************************************************************************************************************** !
  ! SETUP GEMlite
  SUBROUTINE initialise_gemlite(dum_dsc, dum_k1, dum_dz, dum_dza, dum_sv, &
       & dum_sfxsumsed1, dum_sfxsumrok1_gem, dum_sfxsumatm1_gem)
    IMPLICIT NONE
    REAL,                      INTENT(IN)    :: dum_dsc
    INTEGER, DIMENSION(:,:),   INTENT(IN)    :: dum_k1
    REAL,    DIMENSION(:),     INTENT(IN)    :: dum_dz, dum_dza
    REAL,    DIMENSION(:),     INTENT(IN)    :: dum_sv
    REAL,    DIMENSION(:,:,:), INTENT(INOUT) :: dum_sfxsumsed1
    REAL,    DIMENSION(:,:,:), INTENT(INOUT) :: dum_sfxsumrok1_gem
    REAL,    DIMENSION(:,:,:), INTENT(INOUT) :: dum_sfxsumatm1_gem
    ...
    CALL check_iostat(alloc_error,__LINE__,__FILE__)
    ! Copies of GOLDSTEIN data.
    ALLOCATE(goldstein_k1(n_i, n_j), STAT=alloc_error)
    CALL check_iostat(alloc_error, __LINE__, __FILE__)
    ALLOCATE(goldstein_dz(n_k), STAT=alloc_error)
    CALL check_iostat(alloc_error, __LINE__, __FILE__)
    ALLOCATE(goldstein_dza(n_k), STAT=alloc_error)
    CALL check_iostat(alloc_error, __LINE__, __FILE__)
    ALLOCATE(goldstein_sv(0:n_j), STAT=alloc_error)
    CALL check_iostat(alloc_error, __LINE__, __FILE__)
  ...
  SUBROUTINE step_gemlite(dum_sfcocn1, dum_sfxsumsed1, dum_sfxocn1, dum_sfxsumrok1_gem,&
       & dum_sfxsumatm1_gem)
    use gem_cmn
    use gem_util
    use gem_carbchem
    ...
    if (atm_select(ias_pO2)) then
       ...
       datm(ia_pO2,:,:)     = datm(ia_pO2,:,:)     + loc_datm_pO2(:,:)
       datm_sum(ia_pO2,:,:) = datm_sum(ia_pO2,:,:) + loc_datm_pO2(:,:)
       atm(ia_pO2,:,:)      = atm(ia_pO2,:,:)      + loc_datm_pO2(:,:)
    endif
    ...
    DO i=1,n_i
       DO j=1,n_j
          ...
          DO ia = 3, nt_atm
             ...
             datm_sum(ia,i,j) = datm_sum(ia,i,j) + loc_datm_flux
          end do
          ! (2) ocean
