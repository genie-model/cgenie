...
!========================================
subroutine get4d_data_nc(ncid, varname, &
     & dim1, dim2, dim3, dim4, arrayout, ifail)
  implicit none
  ...
  integer, intent(in)      :: dim1 &    ! Size of array in 1st dimension
       ...
       & , dim4      ! Size of array in 4th dimension
  real, intent(out) :: arrayout(dim1,dim2,dim3, dim4) ! the output array
  !----------------------------------------
  ...
  status=nf_inq_dimlen(ncid, dimids(4), dim4nc)
  if (status .ne. nf_noerr) then
     write(6, *) 'ERROR: Could not get 4th dimension from ' &
          & //'netcdf file'
     stop
  endif
  if (dim4nc.ne.dim4) then
     write(6, *) 'ERROR: 4th dimension of variable in model ' &
          & //'and netcdf file do not match'
     write(6, *) 'model and netcdf dims are ', dim4, ' and ' &
          & , dim4nc
     stop
  endif
  !----------------------------------------
  ...
  if (realkind.eq.4) then
     ...
     print*,'precision problem in get4d_data_nc'
     stop
  endif
  if (status .ne. nf_noerr) then
