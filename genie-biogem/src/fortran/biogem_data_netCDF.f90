...
CONTAINS
  ...
  SUBROUTINE sub_save_netcdf(dum_yr,dum_dd)
    ...
    if(.not. loc_defined) then
       ...
       SELECT CASE (dum_dd)
       CASE (2)
          call sub_putvar1d ('lat_moc', loc_iou, n_j, loc_ntrec, n_j, &
...
!!$       !!
!!$       !! GHC 14/9/09 save output to file for use in auto-resubmitting experiments from spin-ups
!!$       !! set loc_ij(:,:) to 0.0 so as not to have *s all over the place!
!!$       loc_ij(:,:) = 0.0
!!$       DO i=1,n_i
!!$          DO j=1,n_j
!!$             IF (n_k >= goldstein_k1(i,j)) THEN
...
!!$       open(out,file=TRIM(par_outdir_name)//'CaCO3toPOCrainratio'//string_results_ext, &
!!$            & action='write',status='replace',form='formatted',iostat=ios)
!!$       DO j=1,n_j
!!$          write(out,fmt='(1000f8.5)') ( loc_ij(i,n_j+1-j), i=1,n_i )
!!$       END DO
...
!!$       !!
    ...
    If (ctrl_data_save_slice_ocn .AND. ctrl_data_save_slice_diag_geochem) then
       IF (ocn_select(io_Fe)) THEN
          ...
          ! nutrient limitation
          ! NOTE: -1.0 equates to complete PO4 limitation, +1.0 to complete Fe limitation
          ...
          if (sed_select(io_PO4)) then
             ...
             DO i=1,n_i
                DO j=1,n_j
                   IF (n_k >= goldstein_k1(i,j)) THEN
                      loc_PO4 = ocn(io_PO4,i,j,n_k)
                      loc_kPO4 = loc_PO4/(loc_PO4 + par_bio_c0_PO4)
                      loc_FeT = ocn(io_Fe,i,j,n_k) + ocn(io_FeL,i,j,n_k)
                      loc_kFe = loc_FeT/(loc_FeT + par_bio_c0_Fe)
                      loc_ij(i,j) = (1.0 - loc_kFe) - (1.0 - loc_kPO4)
                   end IF
                END DO
             END DO
             loc_unitsname = 'n/a'
    ...
    if (ctrl_data_save_slice_bio .AND. ctrl_data_save_slice_diag_bio) then
       ! Biological PO4 uptake
       loc_unitsname = 'mol kg-1 yr-1'
       loc_ij(:,:) = int_diag_bio_timeslice(idiag_bio_dPO4,:,:)/int_t_timeslice
       call sub_adddef_netcdf(loc_iou,3,'bio_diag_dPO4','Biological PO4 uptake', &
            & trim(loc_unitsname),const_real_zero,const_real_zero)
       call sub_putvar2d('bio_diag_dPO4',loc_iou,n_i,n_j,loc_ntrec,loc_ij(:,:),loc_mask_surf)
       ! Temperature limitation of productivity
       loc_unitsname = 'n/a'
       loc_ij(:,:) = int_diag_bio_timeslice(idiag_bio_kT,:,:)/int_t_timeslice
       call sub_adddef_netcdf(loc_iou,3,'bio_diag_kT','Temperature limitation of productivity', &
            & trim(loc_unitsname),const_real_zero,const_real_zero)
       call sub_putvar2d('bio_diag_kT',loc_iou,n_i,n_j,loc_ntrec,loc_ij(:,:),loc_mask_surf)
       ! Light limitation of productivity
       loc_unitsname = 'n/a'
       loc_ij(:,:) = int_diag_bio_timeslice(idiag_bio_kI,:,:)/int_t_timeslice
       call sub_adddef_netcdf(loc_iou,3,'bio_diag_kI','Light limitation of productivity', &
            & trim(loc_unitsname),const_real_zero,const_real_zero)
       call sub_putvar2d('bio_diag_kI',loc_iou,n_i,n_j,loc_ntrec,loc_ij(:,:),loc_mask_surf)
       ! Nutrient limitation of productivity
       loc_unitsname = 'n/a'
       loc_ij(:,:) = int_diag_bio_timeslice(idiag_bio_kN,:,:)/int_t_timeslice
       call sub_adddef_netcdf(loc_iou,3,'bio_diag_kN','Nutrient limitation of productivity', &
            & trim(loc_unitsname),const_real_zero,const_real_zero)
       call sub_putvar2d('bio_diag_kN',loc_iou,n_i,n_j,loc_ntrec,loc_ij(:,:),loc_mask_surf)
       ! DOM fraction
       loc_unitsname = 'n/a'
       loc_ij(:,:) = int_diag_bio_timeslice(idiag_bio_DOMfrac,:,:)/int_t_timeslice
       call sub_adddef_netcdf(loc_iou,3,'bio_diag_DOMfrac','DOM fraction', &
            & trim(loc_unitsname),const_real_zero,const_real_zero)
       call sub_putvar2d('bio_diag_DOMfrac',loc_iou,n_i,n_j,loc_ntrec,loc_ij(:,:),loc_mask_surf)
...
!!!!!!!!!!!!!!!!!!!!
       ! (11) barotropic streamfunction
       loc_unitsname = 'Sv'
       call sub_adddef_netcdf(loc_iou,3,'phys_psi','Barotropic streamfunction',trim(loc_unitsname),const_real_zero,const_real_zero)
       call sub_putvar2d('phys_psi',loc_iou,n_i,n_j,loc_ntrec,int_psi_timeslice(1:n_i,1:n_j)/int_t_timeslice,loc_mask_surf)
!!!!!!!!!!!!!!!!!!!!
    end if
    !----------------------------------------------------------------
    ...
    !----------------------------------------------------------------
    !       WATER-COLUMN INTEGRATED PRODUCTION/OXIDATION RATE
    !----------------------------------------------------------------
    If (ctrl_data_save_slice_ocn .AND. ctrl_data_save_slice_diag_geochem) then
    ...
    !---------------------------------------------------------------- 
    If (ctrl_data_save_slice_misc) CALL sub_save_netcdf_2d_USER()
    !---------------------------------------------------------------- 
    ...
    If (ctrl_data_save_slice_carb) then
       IF (opt_select(iopt_select_carbchem)) THEN
          ...
          DO ic=1,n_carb
             ...
             call sub_adddef_netcdf(loc_iou,3,'carb_'//trim(string_carb(ic)), &
                  & 'bottom-water '//trim(string_carb(ic)),trim(loc_unitsname),const_real_zero,const_real_zero)
             call sub_putvar2d('carb_'//trim(string_carb(ic)),loc_iou,n_i,n_j,loc_ntrec,loc_ij,loc_sed_mask)
          END DO
       end if
    end if
    !----------------------------------------------------------------
  ...
  SUBROUTINE sub_save_netcdf_3d()
    ...
    !       USED-DEFINED (MISC) FIELDS
    !---------------------------------------------------------------- 
    If (ctrl_data_save_slice_misc) CALL sub_save_netcdf_3d_USER()
    !---------------------------------------------------------------- 
  ...
  SUBROUTINE sub_save_netcdf_goldstein_opsi()
    ...
    INTEGER::loc_iou, loc_ntrec
    REAL::loc_scale
    real,DIMENSION(0:n_j,0:n_k)::loc_mask_surf,loc_tmp_jk
    !-----------------------------------------------------------------------
    !       INITIALIZE LOCAL VARIABLES
    !-----------------------------------------------------------------------
    loc_iou = ncout2d_iou
    ...
    call sub_putvar2d('phys_opsi',loc_iou,n_j+1,n_k+1,loc_ntrec,loc_tmp_jk,loc_mask_surf)
    ...
    case ('worbe2', 'worjh2', 'worjh4', 'worlg2', 'worlg4', 'wv2jh2', 'wv3jh2', 'worri4')
       ...
       call sub_putvar2d('phys_opsia',loc_iou,n_j+1,n_k+1,loc_ntrec,loc_tmp_jk,loc_mask_surf)
       ! Pacific
       ...
       call sub_putvar2d('phys_opsip',loc_iou,n_j+1,n_k+1,loc_ntrec,loc_tmp_jk,loc_mask_surf)
    end select
    !-----------------------------------------------------------------------
  ...
  SUBROUTINE sub_save_netcdf_ocn_col_extra()
    ...
    INTEGER::i,j,k
    INTEGER::loc_iou,loc_ntrec
    REAL,DIMENSION(n_i,n_j,n_k)::loc_colbminusr,loc_colboverr, loc_mask
