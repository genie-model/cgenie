function [] = make_runoff_rnd(k1name)
...
%   ***********************************************************************
%
%   13/01/02: fixed bug in how lon wrap-around boundary cells are assigned
%
%   ***********************************************************************
...
% save data - k1 file
fid = fopen([k1name '.RNDRUNOFF.k1'],'w');
for i = 1:i_max
    for j = 1:j_max
        if k1_new(i,j) < 10
...
fid = fopen([k1name '.RNDRUNOFF_BASINS.dat'],'w');
for i = 1:i_max
    for j = 1:j_max
        if k1_col(i,j) < 10
