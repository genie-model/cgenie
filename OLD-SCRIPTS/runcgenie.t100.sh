...
if [ -z "$1" ]; then
    echo "Usage: '$1' 2nd parameter must be the base configuration ID"
    exit 65
  else
    MODELID="$1"
fi
...
if [ -z "$2" ]; then
    echo "Usage: '$2' 3rd parameter must be the user (experiment) configuration file directory"
    exit 65
  else
    GOINDIR=$HOMEDIR/cgenie/genie-userconfigs/"$2"
fi
# [3] set run ID (input run ID (= user configuration file name))
if [ -z "$3" ]; then
    echo "Usage: '$3' 4th parameter must be the run ID"
    exit 65
  else
    RUNID="$3"
fi
...
# [4] set run duration
if [ -z "$4" ]; then
    echo "Usage: '$4' 5th parameter must be the run length (years)"
    exit 65
  else
    RUNLENGTH="$4"
fi
...
CONFIGPATH=$HOMEDIR/cgenie/genie-main/configs
CONFIGNAME=$RUNID".config"
BINARYPATH=$HOMEDIR/cgenie/genie-main
RESTARTNAME="rst.1"
#
# (3) CHECK PARAMETERS
# --------------------------------------
...
if [ -n "$5" ]; then
  ...
  echo ents_2=$OUTPUTPATH"/ents" >> $CONFIGPATH/$CONFIGNAME
